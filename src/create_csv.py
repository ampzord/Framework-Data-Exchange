...
"""
Creates solution.csv file of all the logs retrieved.

Saves the average and standard deviation of all the saved data inside
/logs/ folder to solution.csv to be further analyzed.
"""
...
def sortFoldersFunction(list):
    ...
    for i in range(0, len(list)-1):
        for j in range(len(list)-1):
            if findLowestFolder(list[j], list[j+1]):
                temp = list[j]
                list[j] = list[j + 1]
                list[j + 1] = temp
    return list
...
if __name__ == "__main__":
    ...
    """
    Parses files in /logs/ folder and creates a solution.csv with average and 
    standard deviation of the results retrieved from master.log and client1.log to clientN.log.
    """
    ...
    for root, dirs, files in os.walk(cwd_logs):
        ...
        for dir in dirs:
            ...
            sortedDirs = sortFoldersFunction(dirs) # Sort is needed to reach standardized order of display
            ...
            # ALL CLIENTS
            for i in range(15): # Made for 15 Clients
            ...
            #convert tuple to list -> add to list -> convert to tuple
