...
def on_connect(client, userdata, message, return_code):
    ...
    if return_code != 0:
        logging.info("Master - Error connecting to Broker. Return Code: ", return_code)
        client.reconnect()
    else:
        ...
        if RECEIVED_ALL_DATA_FROM_CLIENT_TOTAL == int(CLIENT_SIZE):
            ...
            for tmp_list in solution_string:
                ...
                for item in tmp_list:
                    if str(client_id_temp) + ',' == tmp_list[j]:  # Found list and value
                        # Make get_information of next element in list
                        if len(solution_string[i]) > 1:  # keeps going
...
def init_logging_config():
    """
    Initiates the logging configuration for master.log
    """

    logging.root.handlers = []

    if 'DEBUG_MODE' in sys.argv:
        logging.basicConfig(filename=SOLUTION_PATH + '\\' + 'master.log',
...
def clientNumberHighestDivisor(clients):
    """
    Returns the number of clients per group and the number of concurrent clients for Self-Organizing Approach
    """
    highest_divisor = clients - 1
    while not clients % highest_divisor == 0:
        highest_divisor -= 1

    clients_per_grp = int(clients / highest_divisor)
    concurrent_clients = highest_divisor
    solution_string = []
...
def getRequestListOfClients(nested_list):
    """
    Returns a list from the first elements inside a nested_list
    """
    new_list = []
