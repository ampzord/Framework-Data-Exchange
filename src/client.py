...
def welding_workflow():
    ...
    while MACHINE_WORKFLOW_CYCLE:
        ...
        if ITERATOR_GENERATE_DATA >= NUMBER_ITERATIONS_TILL_WRITE:
            ...
            GEN_THREAD_TIME_DATA.append(sum(GEN_AUXILIAR_LIST))
            GEN_THREAD_ITERATION_DATA.append(GEN_THREAD_ITERATION_AUX)
...
def information_requested(decoded_message):
    """
    Checks if the current ClientID was requested to send information to Master

    :returns: False if Client was not requested to send data to Master.
    :returns: True if Client was requested to send data to Master.
    """
    if "GET_INFORMATION_" in decoded_message:
        # parse decoded message
        split_string = decoded_message.split("_")
        if "," in split_string[2]:
            split_of_comma_string = split_string[2].split(",")
